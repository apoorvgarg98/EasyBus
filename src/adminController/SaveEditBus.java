...
@WebServlet("/SaveEditBus")
public class SaveEditBus extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int status=BusCRUD.edit(b,oid);
		response.setContentType("text/html");
		String mess;
		if(status==1)
		{
			request.getRequestDispatcher("adminAccount.jsp").include(request,response);
			mess="Bus details successfully edited";
			response.getWriter().println(mess);
		}
		else
		{
			request.getRequestDispatcher("adminAccount.jsp").include(request,response);
			mess="Bus details couldn't be edited.Try again later.";
			response.getWriter().println(mess);
		}
	  }
	}
